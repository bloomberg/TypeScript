// genericFunctionsAndConditionalInference.d.ts
type Boxified<T> = {
    [P in keyof T]: {
        value: T[P];
    };
};
declare function unboxify<T>(obj: Boxified<T>): T;
declare function foo<U, V>(obj: {
    u: {
        value: U;
    };
    v: {
        value: V;
    };
}): {
    u: U;
    v: V;
};
declare let qq: {
    u: number;
    v: string;
};
interface Targets<A> {
    left: A;
    right: A;
}
type Target = keyof Targets<any>;
type Result<F extends Target, A> = Targets<A>[F];
type LR<F extends Target, L, R> = [F] extends ["left"] ? L : R;
interface Ops<F extends Target> {
    _f: F;
    str: Result<F, string>;
    num: Result<F, number>;
    lr<I, O>(a: Result<F, I>, o: Result<F, O>): Result<F, LR<F, I, O>>;
    dict: <P>(p: {
        [k in keyof P]: Result<F, P[k]>;
    }) => Result<F, P>;
}
declare const left: Ops<"left">;
declare const right: Ops<"right">;
declare const ok: <F extends keyof Targets<any>>(at: Ops<F>) => {
    lr: Result<F, LR<F, string, number>>;
};
declare const orphaned: <F extends keyof Targets<any>>(at: Ops<F>) => Result<F, {
    lr: LR<F, string, number>;
}>;
declare const leftOk: {
    lr: string;
};
declare const leftOrphaned: {
    lr: string;
};
declare const rightOk: {
    lr: number;
};
declare const rightOrphaned: {
    lr: number;
};

// ==================
// Original test file: tsc-tests/updated-tests/compiler/genericFunctionsAndConditionalInference.ts
// type Boxified<T> = { [P in keyof T]: { value: T[P]} };
// 
// declare function unboxify<T>(obj: Boxified<T>): T;
// 
// function foo<U, V>(obj: { u: { value: U }, v: { value: V } }): {
//     u: U;
//     v: V;
// } {
//     return unboxify(obj);
// }
// 
// let qq: {
//     u: number;
//     v: string;
// } = foo({ u: { value: 10 }, v: { value: 'hello'} });  // { u: U, v: V } but should be { u: number, v: string }
// 
// // From #42385
// interface Targets<A> {
//     left: A
//     right: A
// }
// type Target = keyof Targets<any>
// type Result<F extends Target, A> = Targets<A>[F]
// 
// type LR<F extends Target, L, R> = [F] extends ["left"] ? L : R
// 
// interface Ops<F extends Target> {
//     _f: F
//     str: Result<F, string>
//     num: Result<F, number>
//     lr<I, O>(a: Result<F, I>, o: Result<F, O>): Result<F, LR<F, I, O>>
//     dict: <P>(p: {[k in keyof P]: Result<F, P[k]>}) => Result<F, P>
// }
// const left: Ops<"left"> = {} as any
// const right: Ops<"right"> = {} as any
// 
// const ok = <F extends Target>(at: Ops<F>): {
//     lr: Result<F, LR<F, string, number>>;
// } => ({lr: at.lr(at.str, at.num)})
// const orphaned = <F extends Target>(at: Ops<F>): Result<F, {
//     lr: LR<F, string, number>;
// }> => at.dict(ok(at))
// 
// const leftOk: {
//     lr: string;
// } = ok(left)
// const leftOrphaned: {
//     lr: string;
// } = orphaned(left)
// 
// const rightOk: {
//     lr: number;
// } = ok(right)
// const rightOrphaned: {
//     lr: number;
// } = orphaned(right)